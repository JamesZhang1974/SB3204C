   ...
   cdrLane((channel-1)*2)     // Clock data recovery mode (1 per GT1724 IC), same as ctLane
{
    ...
    pg = new BertUIPGChannel(QString("PGChannel_%1").arg(channel), parent, channel, usePGLane, 0, 0, 501, 200, showCDRBypassOptions); // 531, 100
    ...
    CONNECT(pg, listPGAmplitudeB_currentIndexChanged(int, int));
    ...
    if (!BertModel::UseFourChanPGMode() && (cdrLane % 4) == 0)
    {
        ...
        CONNECT(cdrModeChannel, listLoopBandwidthSelect_currentIndexChanged(int, int));
        CONNECT(cdrModeChannel, listDataRateSelect_currentIndexChanged(int, int));
        CONNECT(cdrModeChannel, listRateDividerSelect_currentIndexChanged(int, int));
        CONNECT(cdrModeChannel, listTargetLBWSelect_currentIndexChanged(int, int));
